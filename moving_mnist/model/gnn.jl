...
function initEdgeMLP(input_dim, hidden_dim, act_fn)
    ...
    return EdgeMLP(weights, biases, act_fn)
...
function initNodeMLP(node_input_dim, hidden_dim, act_fn, input_dim)
    ...
    return NodeMLP(weights, biases, act_fn)
...
function (t_gnn::TransitionGNN)(states, action)
    ...
    if !t_gnn.ignore_action
        ...
        action_vec = reshape(action, t_gnn.action_dim, batch_size*num_nodes)
        
        #node_attr => [embedding_dim, batch_size*num_nodes]
        #action_vec.shape => [action_dim, batch_size*num_nodes]
        #println(action_vec)

        node_attr = vcat(node_attr, atype(action_vec))
