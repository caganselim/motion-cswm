...
#########################################################################
##################Moving MNIST Velocity Dataset##########################
#########################################################################

struct MovingMNISTVelocityDataset
    """Create dataset of (o_t, a_t, o_{t+1}) transitions from replay buffer."""
...
function buildMovingMNISTVelocityDataset(dataset_path, d_shuffle, batch_size)
    ...
    return  MovingMNISTVelocityDataset(experience_buffer,idx2episode,step, batch_idxs, batch_size)
    
end


function getitem(s::MovingMNISTVelocityDataset,idx)
...
(s::MovingMNISTVelocityDataset)(idx) = getitem(s::MovingMNISTVelocityDataset,idx)

function prepareBatch(s::MovingMNISTVelocityDataset,idx_1, idx_2)
    """Lazy loader to GPU."""
    ...
    b_obs = zeros(50,50,3,minibatch_batch_size)
    b_next_obs = zeros(50,50,3,minibatch_batch_size)
    b_action = zeros(minibatch_batch_size,10)
...
function iterate(s::MovingMNISTVelocityDataset, state = 1)
...
############################################################################
##################Moving MNIST Multi Step Path Dataset######################
############################################################################

struct MovingMNISTMultiStepPathDataset
...
function buildMovingMNISTMultiStepPathDataset(dataset_path, batch_size, step_size)
    
    
    experience_buffer = loadh5file(dataset_path)
    dataset_size = length(experience_buffer)
    
    return MovingMNISTMultiStepPathDataset(experience_buffer, dataset_size, batch_size, step_size)
    
end

function getitem(p::MovingMNISTMultiStepPathDataset,idx)
...
(p::MovingMNISTMultiStepPathDataset)(idx) = getitem(p::MovingMNISTMultiStepPathDataset,idx)

function prepareBatch(p::MovingMNISTMultiStepPathDataset, state)
    """Lazy loader to GPU."""
    ...
    b_obs = zeros(50,50,3,p.batch_size)
    b_next_obs = zeros(50,50,3,p.batch_size)
    b_action = zeros(p.batch_size, 10 ,p.step_size)
...
function iterate(p::MovingMNISTMultiStepPathDataset, state = 1)
